...
MjPage.prototype.mjpage = function(htmlstring, configOptions, typesetOptions, callback) {
    ...
    while (script = scripts[index]) {
         ...
         * @property {string} inputFormat - the source formula format (e.g. "inline-TeX")
        ...
        conf.state.hash = hash;  // for access from typeset callback
        conf.state.parsedFormula = parsedFormula;
        ...
        typeset(conf, (result, options) => {  // async call
            ...
            let jobID = options ? options.state.hash : hash;
            ...
            if (result.errors) {
                ...
                this._outstandingHandlers[jobID]--;
                return;
            ...
            this._parsedFormulasCache[jobID].push(parsedFormula);
...
MjPage.prototype.getOutputProperty = function (conf) {
    let res;
...
exports.mjpage = function (htmlstring, configOptions, typesetOptions, callback) {
    ...
    setImmediate(() => mjpage.mjpage(htmlstring, configOptions, typesetOptions, callback));
